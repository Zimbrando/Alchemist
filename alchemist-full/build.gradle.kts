...
tasks.register("testJpackageOutput") {
    ...
    val dir = rootProject.file("build/package/")
    ...
    doFirst {
        ...
        when {
            isWindows -> {
                exec {
                    commandLine("msiexec", "-i", "${rootProject.name}-$version.msi", "-quiet", "INSTALLDIR=${workingDir.path}\\${windowsOutputDirs["msi"]}")
                    workingDir = rootProject.file("build/package/")
                }
                exec {
                    commandLine("./${rootProject.name}-$version.exe", "-quiet", "INSTALLDIR=${workingDir.path}\\${windowsOutputDirs["exe"]}")
                    workingDir = rootProject.file("build/package/")
                }
            }
            isMac -> println("IM MAC") // commandLine("sudo", "installer", "-pkg", "${rootProject.name}-$version.pkg", "-target", "/")
            else -> {
                dir.resolve("install-rpm").mkdirs()
                dir.resolve("install-deb").mkdirs()
                exec {
        ...
        when {
            isWindows -> {
                windowsOutputDirs.forEach {
                    val execFiles = dir.resolve(it.value).listFiles().map { it.name }
                    val appFiles = dir.resolve("${it.value}/app").listFiles().map { it.name }
            ...
            isMac -> {
                ...
                linuxOutputDirs.forEach {
                    val execFiles = dir.resolve("${it.value}/opt/alchemist/bin").listFiles().map { it.name }
                    val appFiles = dir.resolve("${it.value}/opt/alchemist/lib/app").listFiles().map { it.name }
                    require(rootProject.name in execFiles)
                    require(tasks.jpackage.get().mainJar in appFiles)
                }
            }
